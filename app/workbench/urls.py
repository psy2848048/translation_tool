...
# 원문 목록 조회
workbench.add_url_rule('/<int:did>', view_func=ctrl.get_origin_sentences, methods=['GET'])

# 번역문 목록 조회
workbench.add_url_rule('/origin/<int:oid>/target', view_func=ctrl.get_target_sentences, methods=['GET'])

# 원문 글자수&단어수 조회
workbench.add_url_rule('/<int:did>/cnt', view_func=ctrl.get_doc_sentence_cnt, methods=['GET'])

# 문서 다운로드
workbench.add_url_rule('/<int:did>/output', view_func=ctrl.output_doc_to_file, methods=['GET'])

# 원문 수정
workbench.add_url_rule('/origin/<int:oid>', view_func=ctrl.modify_origin_sentence, methods=['PUT'])

# 원문 삭제
workbench.add_url_rule('/origin/<int:oid>', view_func=ctrl.delete_origin_sentence, methods=['DELETE'])

# 번역문 추가
workbench.add_url_rule('/origin/<int:oid>/target', view_func=ctrl.add_trans_sentence, methods=['POST'])
