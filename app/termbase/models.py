...
def select_termbase(uid, origin_lang, trans_lang, page, rows):
    ...
    results = conn.execute(text("""SELECT tb.id as tbid, origin_lang, trans_lang, origin_text, trans_text 
...
def insert_term(uid, origin_lang, trans_lang, origin_text, trans_text):
    ...
    try:
        ...
        tbid = res.lastrowid
        conn.execute(ut.insert(), user_id=uid, tb_id=tbid)
...
def update_term(tbid, origin_lang, trans_lang, origin_text, trans_text):
    conn = db.engine.connect()
    trans = conn.begin()
    meta = MetaData(bind=db.engine)
    tb = Table('termbase', meta, autoload=True)
    ...
    try:
        if None in [origin_lang, trans_lang, origin_text]:
            trans_hash = common.create_md5_hash(trans_text)
            res = conn.execute(tb.update(tb.c.id == tbid),
                               trans_text=trans_text, trans_hash=trans_hash, update_time=datetime.utcnow())
        else:
            origin_hash = common.create_md5_hash(origin_text)
            trans_hash = common.create_md5_hash(trans_text)
            res = conn.execute(tb.update(tb.c.id == tbid), origin_lang=origin_lang, trans_lang=trans_lang,
                               origin_hash=origin_hash, trans_hash=trans_hash,
                               origin_text=origin_text, trans_text=trans_text, update_time=datetime.utcnow())

        if res.rowcount != 1:
            trans.rollback()
            msg = '{} requests were received.'.format(res.rowcount)
            return msg, -1
...
def delete_term(tbid):
    ...
    try:
        res = conn.execute(tb.update(tb.c.id == tbid), is_deleted=True, update_time=datetime.utcnow())
        if res.rowcount != 1:
            trans.rollback()
            return False, -1
        params = res.last_updated_params()
        deleted_tbid = params['id_1']

        trans.commit()
        return True, deleted_tbid
    except:
        traceback.print_exc()
        trans.rollback()
        return False, -1
