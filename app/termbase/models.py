...
def select_termbase(uid, origin_lang, trans_lang, page, rows):
    ...
    res = conn.execute(text("""SELECT count(*) FROM `marocat v1.1`.termbase
                              WHERE user_id = :uid AND origin_lang = :ol AND trans_lang = :tl AND is_deleted = FALSE ;""")
                       , uid=uid, ol=origin_lang, tl=trans_lang).fetchone()
    total_cnt = res[0]

    results = conn.execute(text("""SELECT id as tid, origin_lang, trans_lang, origin_text, trans_text 
                                  FROM `marocat v1.1`.termbase
...
def insert_term_csv_file(uid, csv_file, origin_lang, trans_lang):
    ...
    try:
        for row in data:
            ...
            if len(row) == 4:
                res = conn.execute(tb.insert(), user_id=uid, origin_lang=row[0], trans_lang=row[1]
